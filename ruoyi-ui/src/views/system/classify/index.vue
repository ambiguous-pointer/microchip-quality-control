<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="调佣阈值" prop="threshold">
        <el-input
      ...
      <el-form-item label="检测" prop="qualityId">
        <el-input
...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="classifyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="调佣阈值" align="center" prop="threshold"/>
      <el-table-column label="分类或检测的置信度" align="center" prop="confidence"/>
      <el-table-column label="分类或检测的类别" align="center" prop="index"/>
      <el-table-column label="分类或检测的类别" align="center" prop="label"/>
...
<!--          </el-button>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="调佣阈值" prop="threshold">
          <el-input v-model="form.threshold" placeholder="请输入调佣阈值"/>
        </el-form-item>
        <el-form-item label="分类或检测的置信度" prop="confidence">
          <el-input v-model="form.confidence" placeholder="请输入分类或检测的置信度"/>
        </el-form-item>
        <el-form-item label="分类或检测的类别" prop="index">
          <el-input v-model="form.index" placeholder="请输入分类或检测的类别"/>
        </el-form-item>
        <el-form-item label="分类或检测的类别" prop="label">
          <el-input v-model="form.label" placeholder="请输入分类或检测的类别"/>
        </el-form-item>
        <el-form-item label="名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入名称"/>
        </el-form-item>
        <el-form-item label="检测编号" prop="qualityId">
          <el-input v-model="form.qualityId" placeholder="请输入检测编号"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除分类识别日志编号为"' + uids + '"的数据项？').then(function () {
        return delClassify(uids);
      }).then(() => {
        this.getList();
